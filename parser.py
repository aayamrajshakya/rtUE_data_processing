...
def parse_csv(csv_input):
    ...
    pivoted_df = df.pivot_table(index=['_time', 'srsue_data_id'], columns='_field', values='_value', aggfunc='first')
    pivoted_df.reset_index(inplace=True)
    columns = ['_time', 'srsue_data_id'] + [col for col in pivoted_df.columns if col not in ['_time', 'srsue_data_id']]
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python3 parser.py {csv_input}")
        sys.exit(1)
